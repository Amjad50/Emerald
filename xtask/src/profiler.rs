...
impl<'a> Sampler<'a> {
    ...
    pub fn get_get(&mut self, name: &str) -> anyhow::Result<u64> {
        let reg = self.qmp.execute(&qmp::human_monitor_command {
            cpu_index: Some(0),
            command_line: format!("print ${name}"),
        })?;

        hex_to_u64(&reg)
    }

    pub fn get_rip(&mut self) -> anyhow::Result<u64> {
        self.get_get("eip")
    }

    pub fn get_rsp(&mut self) -> anyhow::Result<u64> {
        self.get_get("esp")
    }

    pub fn get_rbp(&mut self) -> anyhow::Result<u64> {
        self.get_get("ebp")
    }
    ...
    pub fn sample_with_timing(&mut self, verbose: bool) -> anyhow::Result<(Vec<u64>, Duration)> {
        ...
        if verbose {
            println!(
                "CPU state: RIP=0x{:x}, RSP=0x{:x}, RBP=0x{:x}",
                rip, rsp, rbp
            );
        }
